...
ATreeQueryLayer::ATreeQueryLayer()
{
	PrimaryActorTick.bCanEverTick = false;
}

void ATreeQueryLayer::AddAdditionalMaterial(const ATreeItem* Item, UMaterialInstance* Material)
{
	if (!Material)
	{
		return;
...
void ATreeQueryLayer::BeginPlay()
{
	Super::BeginPlay();
...
void ATreeQueryLayer::CreateLink()
{
...
bool ATreeQueryLayer::HasErrors()
{
...
void ATreeQueryLayer::GetMapComponent()
{
...
void ATreeQueryLayer::OnResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
...
void ATreeQueryLayer::ParseData()
{
	if (FeatureData.IsEmpty())
	{
	...
	if (bGetAllFeatures)
	{
		...
		{
			auto featureItem = GetWorld()->SpawnActor(ATreeItem::StaticClass());
			...
			const auto item = Cast<ATreeItem>(featureItem);
...
void ATreeQueryLayer::ProcessWebRequest()
{
	//FeatureData.Empty();
	FHttpRequestRef Request = FHttpModule::Get().CreateRequest();
	Request->OnProcessRequestComplete().BindUObject(this, &ATreeQueryLayer::OnResponseReceived);
...
void ATreeQueryLayer::RefreshProperties(ATreeItem* Item)
{
...
void ATreeQueryLayer::RemoveAdditionalMaterial(const ATreeItem* Item)
{
	Item->pin->SetOverlayMaterial(nullptr);
}

void ATreeQueryLayer::SelectFeature()
{
	if (APlayerController* PlayerController = Cast<APlayerController>(GetWorld()->GetFirstPlayerController()))
	{
		                                          ...
		                                          EDrawDebugTrace::None, HitResult, true))
		{
			...
			currentFeature = Cast<ATreeItem>(HitResult.GetActor());
...
void ATreeQueryLayer::SpawnFeatures(int Start, int Last)
{
